...
Ollama Cloud 密钥默认是 `MAX`（权重5、单密钥并发8），可在密钥表中改为 `PRO`（权重1、单密钥并发2）。服务每2分钟读取一次官方额度：旧格式使用 5 小时和周额度，新格式使用月额度；混合使用时按各自周期的已用百分比智能均摊，用量相差不超过1个百分点时按模型恢复 MAX/PRO 的5:1分配。账号冷却、暂停或达到固定并发上限时会临时绕开，外部 OpenAI API 不限并发且不参与额度均摊。

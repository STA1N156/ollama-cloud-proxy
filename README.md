...
粘性路由在缓存页面开启，默认关闭。客户端通过 `X-Proxy-Session` 请求头或 OpenAI 请求体的 `user` 字段提供稳定会话标识；绑定只保存在内存中，一小时无请求自动过期。每次请求都会先检查模型支持、并发、五小时 95% 阈值和周额度差，绑定密钥不再合适时会立即重新均衡。关闭时跳过会话哈希和粘性判断，不增加请求路径负担。
